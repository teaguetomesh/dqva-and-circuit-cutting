...
def apply_mixer(circ, alpha, init_state, G, barriers,
    ...
    init_state : str
        The current initial state for the ansatz, bits which are "1"
        are hit with an X-gate at the beginning of the circuit and their partial
        mixers are turned off
    G : NetworkX Graph
...
def apply_phase_separator(circ, gamma, G):
    """
    Apply a parametrized Z-rotation to every qubit
    """
    for qb in G.nodes:
        circ.rz(2*gamma, qb)
...
def gen_dqva(G, P=1, params=[], init_state=None, barriers=1, decompose_toffoli=1,
    ...
    """
    Generate and return the Dynamic Quantum Variational Ansatz (DQVA)
    ...
    verbose : int
        0 is least verbose, 2 is most
    param_lim : int
        Controls the number of parameters allowed in the dqva. If this is set,
    ...
    for i in range(len(alpha_list)):
        alphas = alpha_list[i]
        apply_mixer(dqv_circ, alphas, init_state, G, barriers,
