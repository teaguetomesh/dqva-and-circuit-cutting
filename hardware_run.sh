...
python -m hardware.generator --min-qubit 5 --max-qubit 5 --max-clusters 3 --min-size $MIN_SIZE --max-size $MAX_SIZE --device-name $DEVICE_NAME --circuit-type $CIRCUIT_TYPE 2>&1 | tee -a ./hardware/logs/$CIRCUIT_TYPE\_$DEVICE_NAME\_logs.txt
