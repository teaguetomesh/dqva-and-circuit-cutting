...
#python -m hardware.generator --min-qubit 10 --max-qubit 10 --max-clusters 5 --min-size $MIN_SIZE --max-size $MAX_SIZE --device-name $DEVICE_NAME --circuit-type $CIRCUIT_TYPE 2>&1 | tee -a ./hardware/logs/$CIRCUIT_TYPE\_$DEVICE_NAME\_logs.txt
...
# mpiexec -n 50 python -m utils.reconstructor_parallel --experiment-name hardware --device-name $DEVICE_NAME --circuit-type $CIRCUIT_TYPE --evaluation-method hardware 2>&1 | tee -a ./hardware/logs/$CIRCUIT_TYPE\_$DEVICE_NAME\_logs.txt
