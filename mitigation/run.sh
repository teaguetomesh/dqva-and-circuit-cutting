...
mpiexec -n 11 python generate_evaluator_input.py --min-qubit 2 --max-qubit 9 --max-clusters 3 --device-name $DEVICE_NAME --circuit-type $CIRCUIT_TYPE 2>&1 | tee ./logs/$CIRCUIT_TYPE\_$DEVICE_NAME\_generator_logs.txt
...
echo "Running saturated job submittor"
mpiexec -n 11 python hardware_job_submittor.py --device-name $DEVICE_NAME --circuit-type $CIRCUIT_TYPE --shots-mode saturated 2>&1 | tee ./logs/$CIRCUIT_TYPE\_saturated_$DEVICE_NAME\_hw_job_submittor_logs.txt
echo "Running saturated reconstruction"
...
echo "Running sametotal job submittor"
mpiexec -n 11 python hardware_job_submittor.py --device-name $DEVICE_NAME --circuit-type $CIRCUIT_TYPE --shots-mode sametotal 2>&1 | tee ./logs/$CIRCUIT_TYPE\_sametotal_$DEVICE_NAME\_hw_job_submittor_logs.txt
echo "Running sametotal reconstruction"
