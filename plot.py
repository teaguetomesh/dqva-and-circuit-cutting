...
if __name__ == '__main__':
    ...
    for filename in all_files:
        ...
        plt.figure(figsize=(10,5))
        plt.subplot(121)
        plt.imshow(reduction_map, aspect='auto',extent=[min(hw_qubits_unique)-1, max(hw_qubits_unique)+1, min(fc_qubits_unique)-1, max(fc_qubits_unique)+1], cmap = cm.Greys)
        plt.colorbar()
        plt.title('Cross entropy reduction sweep')
        plt.xlabel('Hardware Qubits')
        plt.ylabel('Full Circuit Qubits')
        plt.xticks(hw_qubits_unique)
        plt.yticks(fc_qubits_unique)
        plt.subplot(122)
