...
def main():
    ...
    parser.add_argument('--circuit-dimension', type=int, nargs="+", default=[4,4,8], metavar='N',
    help='dimension of a quantum supremacy circuit (default: 4 by 4, depth=8)')
    ...
    circ = circuit_generator(args.circuit_dimension,args.layer_order,args.random,args.measure)

    path = '%s/results' % args.home_dir
    if not os.path.isdir(path):
            os.makedirs(path)
    circ.draw(output='text', line_length = 400, filename = '%s/supremacy_circ_%d*%d_%d.txt' % 
    (path, args.circuit_dimension[0], args.circuit_dimension[1], args.circuit_dimension[2]))
    dag_drawer(circuit_to_dag(circ), filename='%s/supremacy_dag_%d*%d_%d.pdf' %
    (path, args.circuit_dimension[0], args.circuit_dimension[1], args.circuit_dimension[2]))
    pickle.dump(circ, open('%s/supremacy_circuit_%d*%d_%d.dump' %(path, args.circuit_dimension[0],args.circuit_dimension[1], args.circuit_dimension[2]), 'wb' ))
