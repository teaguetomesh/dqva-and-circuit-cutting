...
python -m simulator.generator --min-qubit 3 --max-qubit 3 --max-clusters 5 --device-name $DEVICE_NAME --circuit-type $CIRCUIT_TYPE 2>&1 | tee -a ./simulator/logs/$CIRCUIT_TYPE\_$DEVICE_NAME\_logs.txt

mpiexec -n 2 python -m utils.evaluator --experiment-name simulator --device-name $DEVICE_NAME --circuit-type $CIRCUIT_TYPE --shots-mode saturated --evaluation-method noisy_qasm_simulator 2>&1 | tee -a ./simulator/logs/$CIRCUIT_TYPE\_$DEVICE_NAME\_logs.txt
...
mpiexec -n 2 python -m utils.evaluator --experiment-name simulator --device-name $DEVICE_NAME --circuit-type $CIRCUIT_TYPE --shots-mode sametotal --evaluation-method noisy_qasm_simulator 2>&1 | tee -a ./simulator/logs/$CIRCUIT_TYPE\_$DEVICE_NAME\_logs.txt
