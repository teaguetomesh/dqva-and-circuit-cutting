...
python -m simulator.generator --min-qubit 5 --max-qubit 6 --max-clusters 5 --device-name $DEVICE_NAME --circuit-type $CIRCUIT_TYPE
...
mpiexec -n 4 python -m utils.evaluator --experiment-name simulator --device-name $DEVICE_NAME --circuit-type $CIRCUIT_TYPE --shots-mode sametotal --evaluation-method noisy_qasm_simulator
