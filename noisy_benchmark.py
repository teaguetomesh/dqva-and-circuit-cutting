...
for dimension in [[2,4],[3,3],[2,5],[3,4],[2,7]]:
    i,j = dimension
    if i*j<=max_qubit:
        continue
    print('-'*100)
    ...
    # Looking for a cut
    searcher_begin = time()
    hardness, positions, ancilla, d, num_cluster, m = searcher.find_cuts(circ,num_clusters=range(3,max_clusters+1),hw_max_qubit=max_qubit,evaluator_weight=1)
